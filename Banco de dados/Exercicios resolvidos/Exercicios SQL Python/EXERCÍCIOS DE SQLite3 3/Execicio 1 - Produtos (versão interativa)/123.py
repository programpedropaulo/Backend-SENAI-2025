...
# Limpa os dados antigos (opcional - para testes)
...
while True:
    ...
    if opcao == "1":
        ...
        while True:
            preco_input = input("Preço do produto (use ponto ou vírgula): ").replace(",", ".").strip()
            try:
                ...
                print("❌ Preço inválido. Digite um número válido com ponto ou vírgula.")
        ...
        nome_antigo = input("\nDigite o nome exato do produto que deseja atualizar: ").strip()

        cursor.execute("SELECT * FROM Cadastro_de_produtos WHERE nome = ?", (nome_antigo,))
        produto_existe = cursor.fetchone()
        if not produto_existe:
            print("❌ Produto não encontrado! Verifique se digitou corretamente.")
            continue

        tipo_atualizacao = input("Deseja atualizar 'nome', 'preço' ou 'ambos'? ").lower().strip()
        ...
        if tipo_atualizacao == "nome":
            ...
            while True:
                novo_preco_str = input("Digite o novo preço: ").replace(",", ".").strip()
                try:
                    ...
                    print("❌ Preço inválido! Use ponto ou vírgula.")
            ...
            print("❌ Opção inválida! Digite 'nome', 'preço' ou 'ambos'.")
